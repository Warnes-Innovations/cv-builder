...
async function restoreBackendState() {
  ...
  try {
    ...
    // Restore achievement edits and saved decisions into window globals so
    // UI renderers prefer session/user edits before Master_CV data.
    try {
...
async function populateDownloadTab(cvData) {
  ...
  const content = document.getElementById('document-content');
  let html = '<h1>⬇️ File Review</h1><p style="color:#475569;margin-bottom:12px;">Download generated files and review file-level quality checks.</p>';
