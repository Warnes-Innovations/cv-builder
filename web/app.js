...
(function() {
  ...
  window.fetch = async function(...args) {
    ...
    if (resp.status === 409) { showSessionConflictBanner(); }
    return resp;
  };
})();
...
async function fetchJobFromURL() {
  ...
  try {
    ...
    appendMessage('system', `Network error occurred: ${error.message}. Please check your connection or try manual input.`);
...
async function buildPublicationsReviewTable() {
    ...
    if (!dividerInserted && pub.is_recommended === false) {
      ...
      tableHTML += `
        <tr class="pub-divider-row">
          <td colspan="8" style="background:#f3f4f6;color:#6b7280;font-size:0.82em;font-style:italic;
              padding:6px 12px;border-top:2px solid #d1d5db;text-align:center;">
  ...
  // Initialize DataTable — preserve server sort order (recommended first), disable column sorting
  $('#publications-review-table').DataTable({
    paging: false,
    ordering: false,
...
async function submitPublicationDecisions() {
  ...
  // Persist as a structured answer in session state via post-analysis-responses
...
async function backToPhase(step) {
  try {
    ...
    const data = await res.json();
    if (!res.ok || !data.ok) {
      appendMessage('system', '⚠ Could not navigate back: ' + (data.error || 'Unknown error'));
      return;
    }
    ...
    appendMessage('system', '⚠ Network error in backToPhase: ' + err.message);
  }
}
...
async function reRunPhase(step) {
  ...
  try {
    ...
    if (!res.ok || !data.ok) {
      appendMessage('system', '⚠ Re-run failed: ' + (data.error || 'Unknown error'));
      return;
    }
    ...
    removeLoadingMessage(loadingMsg);
    setLoading(false);
    appendMessage('system', '⚠ Network error in reRunPhase: ' + err.message);
  }
}
...
async function showBulletReorder(expId, expTitle) {
  ...
  try {
    ...
    appendMessage('system', '⚠ Could not load bullets: ' + e.message);
    return;
  }
...
async function saveBulletOrder(expId) {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok || !data.ok) {
      appendMessage('system', '⚠ Could not save bullet order: ' + (data.error||'Unknown'));
      return;
    }
    ...
    appendMessage('system', '⚠ Network error saving bullet order: ' + e.message);
  }
}
...
async function resetBulletOrder(expId) {
  try {
    ...
    const data = await res.json();
    if (!res.ok || !data.ok) {
      appendMessage('system', '⚠ Could not reset bullet order: ' + (data.error||'Unknown'));
      return;
    }
    appendMessage('assistant', '↺ Bullet order reset. Relevance-based ordering will apply.');
    document.getElementById('bullet-reorder-modal')?.remove();
  } catch(e) {
    appendMessage('system', '⚠ Network error resetting bullet order: ' + e.message);
  }
}
...
function handleStepClick(step) {
  ...
  // Job step always shown: open load-job panel.
