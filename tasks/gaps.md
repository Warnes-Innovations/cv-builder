...
**Known v1 limitations (logged, not silently dropped):** batch-only reversibility, no per-item undo (deferred to GAP-19/`IMPLEMENTATION_PLAN.md` Phase 16.7); `_harvest_add_skill`/`_harvest_add_summary_variant` imported rather than relocated into `scripts/utils/master_data_mutations.py` (see spec §10 item 5, a legitimate separate refactor); `delete` op unsupported on this path (removals go through the existing structured editors); no client-side resume for an in-progress proposal across a tab switch (state resets on panel re-render by design, since pending proposals live server-side with their own TTL).
