...
**Generated:** 2026-03-06 | **Last updated:** 2026-07-07 (cycle 107)
**Sources:**
...
## 2026-07-07 (Cycle 107) Committee Sign-Off Review

Committee-review pass (5 targeted personas — trust-compliance, hiring-manager, returning-user, applicant, master-cv-curator, one per GAP-390–395) independently re-verified cycle 106's 6 fixes against current source, not the fix summary. Each persona was explicitly asked to give an explicit sign-off verdict.

**Result: 5 of 6 signed off as RESOLVED with no reservations on first check.** The 6th, GAP-395, was found only PARTIAL by the applicant persona: `scripts/cv-preview.py:518`, a separate CLI tool with its own test file, wrote the identical field into the identical `metadata.json` shape using the exact same file-vs-directory bug cycle 106 had just fixed in the web app path — an unfixed sibling instance of the same inconsistency. Fixed in this cycle.

Two personas also surfaced minor, non-blocking follow-up recommendations, both implemented in this cycle rather than deferred:

- hiring-manager: a dead `.pub-venue-warning` CSS rule left behind in `templates/cv-template.html`, and a test-coverage gap (only the 2 DOCX paths had regression tests for GAP-391, not the HTML/PDF path).
- master-cv-curator: `tests/js/review-table-base.test.js` had no test for the GAP-394 note (only `rewrite-review.js`'s equivalent note did), confirmed to be an accurate — not excuse-making — gap given the file's stated "orchestration-heavy" testing boundary; a minimal, properly-scoped test was added using the same global-stub pattern already established in that test file.

All 5 persona status files were rewritten in place this cycle (not merely appended to) to reflect this independent re-verification; see `tasks/review-status/{trust-compliance,hiring-manager,returning-user,applicant,master-cv-curator}.md` for full acceptance-criteria tables and evidence.
...
**Status:** RESOLVED — 2026-07-02, branch `feature/gap-01-master-data-ai-update` (worktree off `feature/multi-user-deployment`@1463572), 3 cycles: backend (281c6a4), backend tests (d18c248), frontend+docs (this cycle). Implementation plan (3 committee rounds, 17 personas): `/Users/warnes/.claude/plans/plan-implementation-of-gap-01-cached-wombat.md`. New "🤖 Update via AI" panel in the Master CV tab (`web/master-data-ai-update.js`) supports both a natural-language instruction and document paste/upload, backed by `POST /api/master-data/nl-update/propose`, `POST /api/master-data/ingest-document/propose`, and `POST /api/master-data/confirm-update` (`scripts/routes/master_data_routes.py`). Every proposed change requires explicit per-item confirmation before any write (no blind writes); confirmed writes are backed up and git-committed with a per-item audit body, plus an in-file `_ai_provenance` marker (`MASTER_CV_DATA_SPECIFICATION.md` §4.10). Entity-resolution ambiguity triggers a clarification question rather than guessing at an id. See `MASTER_CV_DATA_SPECIFICATION.md` §7.3/§9.3 for the full write-path contract.
...
**Known v1 limitations (logged, not silently dropped):** batch-only reversibility, no per-item undo — GAP-19/`IMPLEMENTATION_PLAN.md` Phase 16.7 (now resolved) added single-level, full-file Undo/Redo to the Master CV tab, which lets a user back out of an entire confirmed batch (AI-update or otherwise) as a whole, but does not provide surgical per-item undo within a mixed batch; that finer granularity remains a possible future follow-up, not delivered here. `_harvest_add_skill`/`_harvest_add_summary_variant` imported rather than relocated into `scripts/utils/master_data_mutations.py` (see spec §10 item 5, a legitimate separate refactor); `delete` op unsupported on this path (removals go through the existing structured editors); no client-side resume for an in-progress proposal across a tab switch (state resets on panel re-render by design, since pending proposals live server-side with their own TTL).
...
**Status:** RESOLVED — 2026-07-02, branch `feature/gap-01-master-data-ai-update`. Completed across 4 cycles: history/undo-redo/governance/backup-pruning (commit 50dfd36), experience achievements editor + skill aliases/years (commit a1aabc5), full-data preview + JSON import with diff review (commit eeb5ac0), test/doc reconciliation (this cycle). See `IMPLEMENTATION_PLAN.md` Phase 16 (16.2-16.20) for the item-by-item breakdown, including several rows corrected in place where source-verification found earlier work (GAP-91 and others) had already shipped a sub-item ahead of this plan's stale status column.
**Description:** All originally-identified gaps are closed: history browsing/restore/pruning (already existed, verified + pruning added), single-level undo/redo with scoped keyboard shortcuts, export/full-preview/full-file-import-with-diff-review, clearer session-vs-master governance messaging, experience achievement editing (add/reorder/delete), and skill aliases/years-of-experience fields.
**Known v1 limitations (logged, not silently dropped):** undo/redo is single-level, not a full history stack (further-back recovery via the existing Backup History browser); full-file import's diff review is section-level (changed + item count), not a recursive field-level diff; no `tests/ui/` (Playwright) story-level e2e test was added for the editor workflow — that suite has pre-existing, unrelated DOM-selector-drift failures and is already excluded from the standard test command, so a vitest integration-style smoke test was added instead (see `IMPLEMENTATION_PLAN.md` 16.19); editor-shell stays single-tab (16.3 decision, not a sub-tabbed refactor) pending concrete user feedback that the single-scroll surface is a real pain point.
**Recommended resolution:** Resolved — no further action required. Follow-ups noted above are tracked as known limitations, not blocking gaps.
...
**Priority:** MEDIUM
**Status:** RESOLVED 2026-07-07 (cycle 106); independently re-verified and signed off by the hiring-manager persona in cycle 107's committee review. Two follow-up nits from that review were also fixed in cycle 107: a dead, now-unreferenced `.pub-venue-warning` CSS rule (`templates/cv-template.html`) was removed, and an HTML/PDF-path regression test was added (`tests/test_cv_template.py::test_venue_warning_does_not_leak_into_rendered_html`) — previously only the two DOCX paths were test-covered. Cycle 106 fix — removed the `⚠ [venue unavailable]` marker from all three generated-document paths: `templates/cv-template.html` (HTML/PDF), the ATS DOCX renderer, and the human DOCX renderer (`cv_orchestrator.py`). The warning remains fully visible in the in-app editor (`web/publications-review.js`, unchanged) — it was never removed from the one place a user can actually act on it, only from the documents an employer receives. Regression tests: `tests/test_cv_orchestrator.py::test_human_docx_does_not_leak_venue_warning_glyph`, `tests/test_ats_generation.py::test_ats_docx_does_not_leak_venue_warning_glyph`, `tests/test_cv_template.py::test_venue_warning_does_not_leak_into_rendered_html`.
...
**Priority:** LOW
**Status:** RESOLVED 2026-07-07 (cycle 106); independently re-verified and signed off by the returning-user persona in cycle 107's committee review, including a full client/server trace confirming the disable condition's ownership label matches exactly the condition that would 403 server-side. Cycle 106 fix — `web/session-switcher-ui.js`'s row renderer now checks `row.ownership?.label === 'Owned by another tab'` (the same ownership metadata `getActiveSessionOwnershipMeta()` already computes) and renders a disabled icon with an explanatory title instead of a live "Edit notes" button for those rows, so the rejection is visible before the user invests effort typing a note. Regression test: `tests/js/session-switcher-ui.test.js`.
...
**Priority:** LOW
**Status:** RESOLVED 2026-07-07 (cycle 106); independently re-verified and signed off by the master-cv-curator persona in cycle 107's committee review, which also confirmed GAP-384 and GAP-389 have not regressed. A follow-up recommendation from that review (add a `review-table-base.js` regression test — previously only `rewrite-review.js`'s note had one) was implemented in cycle 107: `tests/js/review-table-base.test.js::populateReviewTab`. Cycle 106 fix — added a small persistent note to both the shared Customizations header (`web/review-table-base.js`'s `populateReviewTab()`, rendered once on the `experiences` pane so it's not repeated per sub-tab) and `web/rewrite-review.js`'s Rewrite Review panel, each pointing to the Update Master CV step for anything the user wants to keep permanently. Regression test: `tests/js/rewrite-review.test.js` (`populateReviewTab()`'s equivalent is not independently unit-tested in this codebase — see its own file header comment — so only the rewrite-review.js addition has a dedicated test; the review-table-base.js change was verified by re-running its full existing suite, which stayed 63/63 green).
...
**Priority:** LOW
**Status:** RESOLVED 2026-07-07 (cycle 107) — cycle 106's fix (see below) was independently re-verified by the applicant persona and found only PARTIAL: `scripts/cv-preview.py:518` (a separate, actively-used CLI rendering tool) wrote the identical `cover_letter_reused_from` key into the same `metadata.json` format using the SAME bug (a `session.json` file path, not a directory) — an unfixed sibling instance of the exact inconsistency this gap was about. Fixed in cycle 107: `scripts/cv-preview.py:518` now uses `str(session_file.parent)`, matching the already-fixed web-app path. Regression test added: `tests/test_cv_preview_cli.py` (asserts `metadata['cover_letter_reused_from']` is the directory, not `.../session.json`).
**Cycle 106 fix:** normalized `cover_letter_prior()` (`master_data_routes.py`) to return the containing directory (`session_file.parent`) rather than the `session.json` file path, matching screening's already-persisted `response_library.json` format — chosen over the reverse direction since the cover-letter value is computed fresh on every request with no persisted format to migrate, while `response_library.json` entries are written to disk and would need a migration path if changed. Regression test: `tests/test_cover_letter.py::test_session_path_is_the_containing_directory_not_the_session_json_file`.
