...
**Executive Summary:** Cycle 7 source review. The highest-impact change this cycle is GAP-181 (RESOLVED): both viewer-panel spell-check submit buttons in `web/spell-check.js` (lines 148 and 271) now read "Generate Preview →", matching the chat-panel button at `index.html:186` and fully closing the label mismatch previously tracked as the #1 first-time-user defect. AC-F3.2 is upgraded from ⚠️ Partial to ✅ Pass for the label consistency sub-issue, though the broader preview-vs-final pipeline invisibility (GAP-79) remains. All 13 workflow step pills continue to display at page load (GAP-78 open), no progress indicator has been added (GAP-14 open), and the "Not ready" LLM badge still reads in red on first view (GAP-76 open). The overall story scorecard moves from 1 Pass / 8 Partial to 2 Pass / 7 Partial.
...
The onboarding modal (`index.html:315–386`) fires on every startup until `cv-builder-welcome-dismissed` is set in localStorage (via `maybeShowWelcomeModal()`). It presents a numbered 3-phase "How it works" summary and branches on master CV state:
...
1. **"Don't show this again" checkbox suppressible in empty state** (`index.html:375`): The checkbox is on `welcome-footer-present`, which is also shown for the `welcome-section-empty` branch (per `session-manager.js`). A user in the empty-profile state can check this box and dismiss the modal before completing onboarding, permanently suppressing the welcome guidance.

2. **LLM "Not ready" badge** (`index.html:53–58`): After modal dismissal, the header shows `"LLM: Loading…"` followed by a ⚠ `"Not ready"` badge with `class="auth-badge unauthenticated"` styled in `color: #fca5a5` (red, `styles.css:40`). A non-technical professional sees this as a system error. The badge tooltip ("No provider/model is configured yet.", set by `_updateLlmStatusPill('unconfigured', ...)` in `ui-core.js:827`) is hover-only. The welcome modal makes no mention of LLM provider setup as a prerequisite. (GAP-76, open.)
...
- **"Rewrites"** (`index.html:125`): Ambiguous — does the user rewrite, or does the AI rewrite for the user? The word makes more sense once you are inside the step.
- **"Harvest"** (`index.html:141`): Domain jargon. Tooltip text "Harvest improvements" (via `title` attribute) is hover-only. The welcome modal step 3 defines it (`index.html:337–340`) but uses the same vocabulary.
- **"LLM:"** in the header (`index.html:53`): Technical acronym never spelled out anywhere in the UI.

No changes to workflow step terminology this cycle.
...
- When the session has no job description, `showLoadJobPanel()` renders a textarea, "Paste the job description here…" placeholder, and three input method tabs (Paste Text / From URL / Upload File). This is adequate once shown.
- The generic empty-state fallback (`index.html:232–236`) reads "Select a tab to view content / Job description and analysis results will appear here" and appears briefly before the job panel loads. It contains no call to action.
- The `🔍 Analyze Job` button (`index.html:182`) is rendered and enabled even before any job description is pasted; clicking it with an empty input produces an error with no adjacent proactive guidance.

No changes to this area this cycle.
...
**Positive:** The secondary tab bar implements progressive disclosure correctly. `updateTabBarForStage()` (`ui-core.js:607–616`) hides all tabs except those listed in `STAGE_TABS[stage]` (`ui-core.js:350–363`). At startup, only the `job` tab is shown. The chat-panel action button row exposes only one primary action at a time via `display:none` toggling (`index.html:183–190`).
...
- **Spell Check stage:** The "no issues found" zero-state (`spell-check.js:135–151`) now reads: "Continue when you are ready to generate your CV." Adequate contextual guidance.
...
No new stage-purpose changes this cycle.
...
The overall transition feedback mechanism is adequate.
...
- The workflow step bar applies `active`, `completed`, and `upcoming` CSS classes (`styles.css:149–154`) but no legend exists in the UI to explain what blue vs. green vs. grey means. `sr-only` spans added in a prior cycle announce state to screen readers, but sighted first-time users without CSS class knowledge have no key.

No changes to completion signaling this cycle.

---

#### AC-F3.2 — The relationship between generation, layout review, and finalisation is understandable

**Rating:** ⚠️ Partial (label consistency improved to ✅ for spell-check buttons specifically)

**Evidence:**

**GAP-181 RESOLVED — spell-check viewer-panel buttons updated:** Both viewer-panel submit buttons in `web/spell-check.js` now read "Generate Preview →":

- `spell-check.js:148` — zero-issues path, inside `renderSpellCheckZeroState()`: `<button class="submit-btn" onclick="submitEmptySpellCheck()">Generate Preview →</button>` ✅
- `spell-check.js:271` — issues-found path, at the end of `renderSpellSuggestions()`: `<button class="submit-btn" onclick="submitSpellCheckDecisions()">Generate Preview →</button>` ✅

This resolves the label mismatch tracked in the previous cycle review. The chat-panel spell-btn (`index.html:186`) and both viewer-panel spell-btn buttons now consistently read "Generate Preview →". A first-time user advancing through spell check via either the chat panel or the viewer panel will see the same accurate label.

**Remaining issues (unchanged):**

1. The three-step generation pipeline (spell check → preview/layout review → confirm → final files) is technically structured via `GENERATION_PHASES` (`state-manager.js:57–62`) but is never communicated in the visible UI before the user begins. There is no "Step 1 of 3: Preview" indicator on the Layout Review screen, and no introductory sentence at the entry to Layout Review explaining that this produces a reviewable preview, not a final file.

2. The transition from "✅ Confirm Layout" to the generation of final files is signaled only by the button label changing after the user presses it. No explanatory text adjacent to the buttons explains that confirming layout triggers a separate final-generation step (`layout-instruction.js:360–381`).
...
**Residual gap:** The label consistency issue (GAP-181) is closed. The pipeline-invisibility issue (GAP-79) remains open: even with "Generate Preview →" as the label, a first-time user who has never been told that a layout confirmation step exists before final generation will be surprised by the intermediate step.
...
- A `#harvest-section` div revealed post-submit (`finalise.js:193–194`), headed "📥 Update Master CV Data" with a "Skip" button — the "Skip" label does signal optionality.
...
| Generate Preview → | ✅ Clear | Chat-panel spell-btn + both viewer-panel spell-btn buttons now consistent (GAP-181 resolved) |
...
## Cycle 7 Delta: What Changed Since Cycle 6

| Item | Cycle 6 finding | Cycle 7 status |
| --- | --- | --- |
| GAP-181: spell-check viewer-panel buttons label | ❌ Both read "Done — Generate CV →" | ✅ RESOLVED — `spell-check.js:148,271` both read "Generate Preview →" |
| AC-F3.2 label mismatch (chat-panel vs viewer-panel) | ⚠️ Partial — chat-panel fixed, viewer-panel not | ✅ RESOLVED for label consistency; GAP-79 (pipeline invisible) remains open |
| AC-F1.1 / AC-F1.2 / AC-F1.3 | All ⚠️ Partial | Unchanged |
| AC-F2.1 / AC-F2.2 | Both ⚠️ Partial | Unchanged |
| AC-F2.3 | ✅ Pass | Unchanged |
| AC-F3.1 / AC-F3.3 | Both ⚠️ Partial | Unchanged |
...
| GAP-79 Preview vs. final pipeline invisible | Open | Partially mitigated by label fix; pipeline itself still unexplained |
...
6. **[US-F1 / AC-F1.1 — ⚠️ risk]** The "Don't show this again" checkbox (`index.html:375`) is visible in the empty-profile state. A user who checks it before completing onboarding will permanently suppress the welcome guidance.
...
- US-F1 AC-F1.1: welcome modal branches → `web/index.html:315–386`
...
- US-F2 AC-F2.1: stage tab progressive disclosure → `web/ui-core.js:350–363`, `607–616`
...
- US-F3 AC-F3.2: GAP-181 RESOLVED — spell-btn viewer-panel → `web/spell-check.js:148` ("Generate Preview →"), `web/spell-check.js:271` ("Generate Preview →")
- US-F3 AC-F3.2: spell-btn chat-panel → `web/index.html:186` ("Generate Preview →")
