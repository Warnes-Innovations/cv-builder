...
| ATS-safe fonts (Arial/Calibri/Times New Roman) | ⚠️ Partial | `cv_orchestrator.py:3836–3867` — `_setup_ats_styles` sets font sizes on Heading 1, Heading 2, List Bullet but does NOT set `doc.styles['Normal'].font.name`. The human DOCX explicitly sets `font.name = 'Calibri'` at line 4369 (commit b7fb7c5 added Heading style calls only to the human DOCX helper `_heading`, not to the ATS path). ATS DOCX relies on python-docx default (Calibri in practice but not guaranteed). (GAP-H8) |
...
| Job entry on one line: Title \| Company \| Location \| Date Range | ✅ Pass | `cv_orchestrator.py:3767–3779` — pipe-separated single bold run with all four fields |
...
| 9 — Keyword density not stuffed | 🔲 Not Impl | No keyword density ceiling implemented. (GAP-H9) |
...
| Score updates live as user approves/rejects items — no page reload | ✅ Pass | `ats-refinement.js:211–213` — `scheduleAtsRefresh` with 600 ms debounce; triggered from skills-review (`skills-review.js:1081`), experience-review (`experience-review.js:342`), rewrite-review (`rewrite-review.js:483`), spell-check (`spell-check.js:169,438`), summary-review (`summary-review.js:208,236,304`) |
...
**Terminology observation:** The ATS Score tab label in `index.html:211` reads "📊 ATS Score" — this is the same emoji icon used on the Experiences tab ("📊 Experiences" at `index.html:204`). The two tabs are visually ambiguous when scanned quickly by a screen reader or keyboard user. Consider using a distinct icon for the ATS Score tab (e.g. "🎯 ATS Score").

**Proposed story: US-H9 — ATS vocabulary guidance during intake**
The applicant receives no guidance at job input time about which keywords to watch for when reviewing the job description. A brief "ATS tips" note when the JD is analyzed (visible on the Analysis tab) would reinforce the workflow's ATS-first philosophy.

---

**Reviewed against:** web/index.html, web/app.js, web/ui-core.js, web/state-manager.js, web/styles.css, web/ats-refinement.js, web/ats-modals.js, scripts/web_app.py, scripts/utils/cv_orchestrator.py, scripts/utils/conversation_manager.py, scripts/utils/scoring.py, scripts/routes/generation_routes.py
...
- US-H8: No `skill_type` UI override → `web/skills-review.js` (no toggle found)
