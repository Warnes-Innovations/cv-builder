...
| Session-free endpoints | ⚠️ Corrected 2026-09-11 | `/api/model-catalog`, `/api/model` | The original row listed `/api/pricing` and `/api/models` and called them "implemented correctly"; neither has ever had a route. The review copied the instruction file rather than checking the code, which is how the error survived a review that recorded a Pass |
