...
- Any approved additional skills are stored in session state for the current customization workflow, and are eligible for write-back to `Master_CV_Data.json` only through the explicit Harvest Apply step during finalisation, with relevant experience.
