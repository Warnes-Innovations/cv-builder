...
class TestMain(unittest.TestCase):
    ...
    def _run(self, argv: list[str], in_cvgen: bool = True) -> tuple[int, list]:
        """Run main() with subprocess.run mocked; return (exit_code, cmd_list)."""
    ...
    def test_passes_json_flag(self):
        _, cmd = self._run(["--json"])
        self.assertIn("--json", cmd)

    def test_passes_custom_output_dir(self):
        _, cmd = self._run(["--output-dir", "/tmp/cv-out"])
